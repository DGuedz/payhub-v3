  ...
  try {
    ...
    if (execEnabled) {
      ...
      try {
        ...
        const amountAObj = amountA ? { currency: String(poolCurrency).toUpperCase(), issuer: poolIssuer, value: String(amountA) } : undefined;
        const amountBObj = amountB ? { currency: String(poolCurrency).toUpperCase(), issuer: poolIssuer, value: String(amountB) } : undefined;
        const tx = { TransactionType: 'AMMDeposit', Account: wallet.address, ...(amountAObj ? { Amount: amountAObj } : {}), ...(amountBObj ? { Amount2: amountBObj } : {}) };
    ...
    try { const { logger } = require('./_logger'); if (logger && typeof logger.logTxAudit === 'function') logger.logTxAudit(txHash || 'simulated', sequence || -1, { type: 'AMMDeposit', poolCurrency, poolIssuer, amountA: amountA || null, amountB: amountB || null }); } catch {}
    const timestamp = new Date().toISOString();
    return res.status(200).json({ ok: true, executed: execEnabled, txHash, sequence, pool: { currency: poolCurrency, issuer: poolIssuer }, amounts: { amountA, amountB }, timestamp });
  } catch (e) {
